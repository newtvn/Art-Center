    ...
    <div v-else class="min-h-screen p-6 md:p-12 font-sans text-zinc-900 relative z-20">
        ...
        <section class="grid grid-cols-1 md:grid-cols-2 gap-12">
            ...
            <div class="border border-zinc-200 p-8 rounded-3xl hover:shadow-xl transition duration-500 bg-white/50 backdrop-blur-md flex flex-col justify-between group">
                <div>
                    <h2 class="text-xs font-bold uppercase tracking-widest mb-8 text-zinc-400">Artist Profile</h2>
                    <div v-if="profile" class="flex items-center gap-6 mb-8">
                        <img :src="profile.photo || 'https://via.placeholder.com/150'" class="w-20 h-20 rounded-apple object-cover grayscale group-hover:grayscale-0 transition duration-500 shadow-lg">
                        <div>
                            <h3 class="text-2xl font-bold mb-1">{{ profile.name }}</h3>
                            <p class="text-[10px] uppercase tracking-widest text-zinc-500">{{ profile.specialty }}</p>
                        </div>
                    </div>
                    <div v-else class="h-20 flex items-center mb-8 opacity-50">
                        <p class="text-sm font-bold text-zinc-400">No profile active.</p>
                    </div>
                    <p class="text-[10px] text-zinc-400 mb-6 leading-relaxed">Manage your public persona. This data reflects on your Curators page.</p>
                </div>
                <button @click="router.push('/admin/profile')" class="w-full bg-black text-white py-4 rounded-full text-[10px] uppercase font-bold tracking-[0.2em] hover:bg-zinc-800 transition shadow-lg group-hover:scale-[1.02]">Edit Profile</button>
            </div>

            <!-- Artworks Section -->
            <div class="border border-zinc-200 p-8 rounded-3xl hover:shadow-xl transition duration-500 bg-white/50 backdrop-blur-md flex flex-col justify-between group">
                <div>
                    <h2 class="text-xs font-bold uppercase tracking-widest mb-8 text-zinc-400">Collection Registry</h2>
                    <div class="h-20 flex items-center gap-4 mb-8">
                         <span class="text-5xl font-bold tracking-tighter">{{ artCount }}</span>
                         <span class="text-[10px] uppercase font-bold tracking-widest text-zinc-400 mt-2 block">Works<br>Cataloged</span>
                    </div>
                    <p class="text-[10px] text-zinc-400 mb-6 leading-relaxed">Draft, archive, and manage Exhibition pieces for the gallery.</p>
                </div>
                <button @click="router.push('/admin/artworks')" class="w-full bg-black text-white py-4 rounded-full text-[10px] uppercase font-bold tracking-[0.2em] hover:bg-zinc-800 transition shadow-lg group-hover:scale-[1.02]">Manage Artworks</button>
            </div>
        </section>
    </div>
</template>
